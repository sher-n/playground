...
const StoreLists = (props) => {
    ...
    const removeProduct=() => {
        axios.delete(`/store/${props.id}`, {}).then(res => { 
    ...
    const submitChangeProductData = () => {

        axios.put(`/store/${props.id}`, {product_name: nameEdit, price: priceEdit
        }).then(res => { 
            console.log(res.data)
              return;
          });
...
{editToggle1? <Input placeholder="change price" onChange={(e) => setPriceEdit(e.target.value)}
    ...
    flexWrap="wrap"
  >
    {editToggle1? <Button5 onClick={submitChangeProductData} value="EDIT" color="white" bg="#9F6D23" variant="solid" /> : <Button5 onClick={() => setEditToggle1(true)} value="EDIT" color="white" bg="#E47711" variant="solid" />}  
    
    <Box w={5}  />
    <Button5 onClick={removeProduct} value="REMOVE" color="white" bg="#DF684E" variant="solid" />
  </Box>
  </GridItem>
  </Grid>
  ))
